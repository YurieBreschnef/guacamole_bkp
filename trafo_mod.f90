...
! XXX   !$omp parallel &
! XXX   !$omp private ( my_thread_id,my_y_start,my_y_end )
! XXX     my_thread_id  = omp_get_thread_num ( )
! XXX     my_y_start    = my_thread_id*(ydim/threads)
! XXX     my_y_end      = my_y_start + (ydim/threads)-1
! XXX     if(my_thread_id==threads-1) my_y_end=my_y_end+mod(ydim,threads)
! XXX     write ( *, * ) 'thread',my_thread_id,'y_start:',my_y_start,'y_end:',my_y_end
! XXX   !$omp end parallel
  ...
  if(dir==1) then
          ...
          !$omp parallel &
          !$omp private ( j,my_thread_id,my_y_start,my_y_end )
          ...
          ! y-pencils:
          !$omp parallel &
          !$omp private ( i,my_thread_id,my_x_start,my_x_end )
              my_thread_id  = omp_get_thread_num ( )
